...
def pitch_dec(frag, num, fs, freq):
	frag = resampy.resample(frag, fs, 16000)
	time, frequency, confidence, activation = crepe.predict(frag, 16000, viterbi=True)

	return frequency

if __name__ == "__main__":
    
    x_1, fs = librosa.load('Sine.wav', sr=44100)
    time, frequency, confidence, activation = crepe.predict(x_1, fs, viterbi=True)
